...
export const getShortName = (data) => (typeof data === 'object' ? data.shortName : data);
...
export const isInArray = (array, id) => array.filter((idx) => isEqual(idx, id)).length > 0;
...
export const formatUrlParams = (params, encode) =>
  Object.keys(params)
    ...
    .map((key) => `${key}=${encode ? encodeURIComponent(params[key]) : params[key]}`)
    .join('&');
