...
class CreatePostForm extends React.Component {
      ...
      Promise.resolve()
        .then(
          () =>
            ...
            Promise.reject(new Error('Please include a message or an image or video.'))
        )
  ...
  render() {
    ...
    const { buttonProps, form, formProps, inputProps, orientationChange, resizeOnUpload, submit } =
      this.props;
            ...
            <GridColumn sm={6}>
              ...
              >
                <InputImage
                  ...
                  onFileChange={(val) => this.handleChangeAttachment(val, 'image')}
                />
              </InputModal>
            </GridColumn>
            <GridColumn sm={6}>
              <InputModal buttonProps={buttonProps} data={video} type="video" width={20}>
                <InputVideo
                  {...inputProps}
                  {...form.fields.video}
                  onChange={(val) => this.handleChangeAttachment(val, 'video')}
                  onVideoChange={(val) => this.handleChangeAttachment(val, 'video')}
                />
              </InputModal>
            </GridColumn>
          </Grid>
