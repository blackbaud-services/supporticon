...
import { deserializePage, fetchPages } from '../pages';
...
export const deserializeTeam = (team) => {
  ...
  return {
    ...
    leader: [get(team, 'captain.firstName'), get(team, 'captain.lastName')].join(' '),
...
export const deserializeTeamPage = (page) => {
  ...
  const totals = deserializeTotals(get(page, 'totals.timeline', []), currencyCode);
...
const getPaginatedMembers = (team) => {
  return new Promise((resolve) => {
    if (team.membership.members.length < team.membership.numberOfMembers) {
      return client
        .get(`/v1/teamsv3/${team.shortName}?nextPageKey=${team.pagination.endCursor}`)
        .then((res) => {
          const updatedTeam = {
            ...team,
            membership: {
  ...
  return client
    ...
    .then((team) => {
      if (options.includeFitness) {
        return fetchTeamFitness(team.teamGuid, options.fitnessParams).then((fitness) => ({
          ...team,
          fitness,
        }));
      }
      ...
      if (options.includePages) {
        return getPaginatedMembers(team).then((updatedTeam) => {
          if (options.includeFullPages) {
            const ids = updatedTeam.membership.members.map((p) => p.fundraisingPageGuid);
  ...
  return client.servicesAPI
    ...
    .then((data) => {
      if (data.totalCount > 10) {
        ...
        const pageCursors = times(Math.ceil(Math.min(data.totalCount, limit) / 10) - 1, (integer) =>
          encodeBase64String(integer * 10 + 9)
        );
...
export const checkTeamSlugAvailable = (
  ...
  return client.servicesAPI
    .get(`/v1/justgiving/proxy/campaigns/v1/teams/shortNames/${slug}/isAvailable`, options)
...
export const createTeam = (params) => {
  ...
  return checkTeamSlugAvailable(payload.teamShortName, { authType, token })
    .then((teamShortName) => client.put('/v2/teams', { ...payload, teamShortName }, options))
    .then((res) => (res.errorMessage ? Promise.reject(new Error(res.errorMessage)) : res))
...
export const updateTeam = (
  id = required(),
  { name = required(), token = required(), currency = 'GBP', image, target, story }
) => {
  const headers = { Authorization: `Bearer ${token}` };
  const prepareStory = (story) =>
    `[{"type":"paragraph","nodes":[{"type":"text","ranges":[{"text":"${story}"}]}]}]`;
  const update = (url, data) =>
    client.servicesAPI.put(`/v1/justgiving/proxy/campaigns/v1/teams/${id}/${url}`, data, {
      headers,
    });
