...
describe('Fetch Leaderboards', () => {
  ...
  it.skip('uses the correct url to fetch a campaign leaderboard', (done) => {
    ...
    moxios.wait(() => {
      ...
      expect(request.url).to.contain('/donationsleaderboards/v1/leaderboard?campaignGuids=1234');
      done();
    });
  });
  ...
  it('uses the correct url to fetch a campaign leaderboard with all pages', (done) => {
    ...
    moxios.wait(() => {
      const request = moxios.requests.mostRecent();
      expect(request.url).to.contain('/v1/justgiving/campaigns/my-campaign/pages');
      done();
    });
  });
  ...
  it('uses the correct url to fetch an event leaderboard', (done) => {
    ...
    moxios.wait(function () {
      const request = moxios.requests.mostRecent();
      expect(request.url).to.equal('/v1/events/leaderboard?eventid=12345&currency=GBP');
      done();
    });
  });
  ...
  it('allows ther country (and currency) to be set', (done) => {
    ...
    moxios.wait(function () {
      const request = moxios.requests.mostRecent();
      expect(request.url).to.equal('/v1/events/leaderboard?eventid=12345&currency=AUD');
      done();
    });
  });
  ...
  it.skip('uses the correct url to fetch a charity leaderboard', (done) => {
    ...
    moxios.wait(function () {
      const request = moxios.requests.mostRecent();
      expect(request.url).to.include('/donationsleaderboards/v1/leaderboard?charityIds=4567');
      done();
    });
  });
