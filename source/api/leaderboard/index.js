...
import { currencyCode, currencySymbol } from '../../utils/currencies';
...
export const fetchLeaderboard = (params = required()) => {
  ...
  if (params.tagId || params.tagValue || params.sortBy) {
    return getGraphQLeaderboard({
      ...
      .then((results) => removeExcludedPages(results, params.excludePageIds))
      .then((results) => results.filter((item) => lodashGet(item, 'amounts[0].value', 0) > 0));
  }
...
export const fetchEventLeaderboard = (params) => {
  if (params.type === 'team') {
    return Promise.reject(new Error('Team leaderboards by event are not supported'));
  }

  const args = {
    eventid: Array.isArray(params.event) ? params.event.map(getUID) : getUID(params.event),
  ...
  return get(
    ...
    },
    {
      ...
      transforms: {
        ...
        page: (val) => String(val ? Math.min(maxPerRequest, params.limit || 10) * (val - 1) : 0),
  ...
  return results.map((result) => {
    return isTeam
      ? {
          ...result,
          ...result.team,
          currencyCode: lodashGet(result.team, 'fundraisingConfiguration.currencyCode'),
          donationAmount:
            lodashGet(result, 'team.donationSummary.totalAmount') || result.donationAmount,
          eventName: [result.team.captain.firstName, result.team.captain.lastName].join(' '),
          ...
          target: lodashGet(result.team, 'fundraisingConfiguration.targetAmount'),
          type: 'team',
        }
      : {
          ...result,
          ...result.page,
          donationAmount: lodashGet(result, 'page.raisedAmount') || result.donationAmount,
          eventName: [result.page.owner.firstName, result.page.owner.lastName].join(' '),
          pageId: result.id,
...
const recursivelyFetchJGLeaderboard = (campaign, q, limit = 10, results = [], page = 1) => {
  if (campaign.split(',').length > 1) {
    return Promise.all(campaign.split(',').map((id) => recursivelyFetchJGLeaderboard(id, q, limit)))
      .then(flatten)
  ...
  return servicesAPI
    ...
    .then((data) => {
      ...
      return recursivelyFetchJGLeaderboard(campaign, q, limit, updatedResults, page + 1);
    });
};
...
export const deserializeLeaderboard = (supporter, index) => {
  const isTeam =
    supporter.type === 'team' || lodashGet(supporter, 'slug', '').indexOf('team/') === 0;
  const slug =
    supporter.pageShortName ||
    supporter.shortName ||
    (supporter.slug ? supporter.slug.replace(/(fundraising|team)\//, '') : undefined);
  const owner =
    lodashGet(supporter, 'pageOwner.fullName') || lodashGet(supporter, 'owner.firstName')
  ...
  return {
    currency:
      supporter.currencyCode || lodashGet(supporter, 'donationSummary.totalAmount.currencyCode'),
    ...
    subtitle: owner || supporter.eventName || lodashGet(supporter, 'owner.name'),
    target:
    ...
    url:
      supporter.url || slug
        ? [
            baseUrl(),
            slug.indexOf('page/') === -1 ? (isTeam ? 'team' : 'fundraising') : 'page',
            slug.replace('page/', ''),
          ].join('/')
        : undefined,
  };
