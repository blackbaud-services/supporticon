...
import { deserializeDonationTotals, fetchDonationTotals } from '..';
  ...
  describe('Fetch Donation Totals', () => {
    ...
    it('uses the correct url to fetch totals for an event', (done) => {
      ...
      moxios.wait(() => {
        ...
        expect(firstRequest.url).to.equal('/v1/justgiving/donations?eventId=12345');

        expect(secondRequest.url).to.equal('/v1/events/leaderboard?eventid=12345&currency=GBP');
        done();
      });
    });
    ...
    it('allows the country (and currency) to be specified', (done) => {
      ...
      moxios.wait(() => {
        ...
        expect(request.url).to.equal('/v1/events/leaderboard?eventid=12345&currency=EUR');
        done();
      });
    });
    ...
    it('uses the correct urls to fetch totals for a campaign (with offline amounts)', (done) => {
      ...
      moxios.wait(() => {
        ...
        expect(firstRequest.url).to.equal('/v1/justgiving/campaigns/my-campaign');
        expect(secondRequest.url).to.equal('/v1/justgiving/campaigns/my-campaign/pages');
        done();
      });
    });
    ...
    it('uses the correct url to fetch totals for a charity', (done) => {
      ...
      moxios.wait(() => {
        const request = moxios.requests.mostRecent();
        expect(request.url).to.include('/donationsleaderboards/v1/totals?charityIds=1234');
        done();
      });
    });
