...
describe ('Search Address EDH', () => {
  ...
  it ('uses the correct url', (done) => {
    ...
    moxios.wait(() => {
      ...
      expect(request.url).to.contain('https://everydayhero.com/api/v2/addresses')
...
describe ('Get Address Details EDH', () => {
  ...
  it ('uses the correct url', (done) => {
    ...
    moxios.wait(() => {
      const request = moxios.requests.mostRecent()
      expect(request.url).to.contain('https://everydayhero.com/api/v2/addresses/au/123')
      done()
    })
  })
  ...
  it ('uses the correct url for a different region', (done) => {
    ...
    moxios.wait(() => {
      const request = moxios.requests.mostRecent()
      expect(request.url).to.contain('https://everydayhero.com/api/v2/addresses/us/123')
      done()
    })
  })
})
