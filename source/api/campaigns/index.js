...
export const fetchCampaign = (id = required()) =>
  servicesAPI.get(`/v1/justgiving/campaigns/${id}`).then((response) => response.data);

export const deserializeCampaign = (campaign) => ({
  charities: campaign.charities,
  donateUrl: [baseUrl('link'), 'v1/campaign/donate/campaignGuid', campaign.campaignGuid].join('/'),
  eventId: campaign.eventId,
  getStartedUrl: `${baseUrl()}/fundraising-page/creation?campaignGuid=${campaign.campaignGuid}`,
