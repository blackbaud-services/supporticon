...
import { fetchCampaign, fetchCampaigns } from '..';
...
describe('Fetch Campaigns', () => {
  ...
  it('fetches multiple campaigns if ids are passed', (done) => {
    ...
    moxios.wait(() => {
      ...
      expect(secondRequest.url).to.equal('/v1/justgiving/campaigns/another-campaign');
      done();
    });
  });
  ...
  it('fetches a single campaign', (done) => {
    ...
    moxios.wait(() => {
      ...
      expect(request.url).to.equal('/v1/justgiving/campaigns/my-campaign');
      done();
    });
  });
